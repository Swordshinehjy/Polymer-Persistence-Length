...
def compute_lp_terpolymer_Tscan(polymer_models,
                                prob_list,
                                T_list,
                                plot=True) -> np.ndarray:
    """
    ...
    Shape: (len(T_list), len(prob_list))
    """
    # Type checking
    if not hasattr(polymer_models, '__iter__'):
        raise TypeError("polymer_models must be iterable")
    ...
    # 1. Collect all G_unit matrices at all temperatures
    #    mats[k] = (N, 5, 5)
    mats = np.stack(
        [m.persistence_length_Tscan(Ts)['G_unit'] for m in model_list],
        axis=0)  # (K, N, 5, 5)
    ...
    # 2. Weighted combination by prob (vectorized)
    #    For each probability set p (shape P,K):
    #    G_avg[p,n,:,:] = sum_k p[p,k] * mats[k,n,:,:]
    G_avg = (prob[:, :, None, None, None] * mats[None]).sum(
        axis=1)  # (P, N, 5, 5)
    # 3. Fully vectorized persistence length calculation
    M = G_avg[:, :, 1:4, 1:4]  # (P, N, 3, 3)
    p_vec = G_avg[:, :, 1:4, 4]  # (P, N, 3) - this extracts column 4 for rows 1-3
    # Create identity matrix for broadcasting
    I = np.eye(3)  # (3, 3)
    I_minus_M = I[None, None, :, :] - M  # (P, N, 3, 3)
    # Vectorized solve using matrix inversion
    # R_avg = (I - M)^{-1} @ p for each system
    try:
        # Compute inverse of all matrices at once
        I_minus_M_inv = np.linalg.inv(I_minus_M)  # (P, N, 3, 3)
        # Matrix multiplication: (P, N, 3, 3) @ (P, N, 3, 1) -> (P, N, 3, 1)
        R_avg = np.matmul(I_minus_M_inv, p_vec[..., None]).squeeze(-1)  # (P, N, 3)
    except np.linalg.LinAlgError:
        # Fallback: solve individually if batch inversion fails
        R_avg = np.zeros((P, N, 3))
        for p_idx in range(P):
            for n_idx in range(N):
                try:
                    R_avg[p_idx, n_idx] = np.linalg.solve(
                        I_minus_M[p_idx, n_idx], p_vec[p_idx, n_idx])
                except np.linalg.LinAlgError:
                    R_avg[p_idx, n_idx] = [np.inf, np.inf, np.inf]
    # Check for rigid rod condition using determinant
    dets = np.linalg.det(I_minus_M)  # (P, N)
    # Project onto x-axis (direction of first bond)
    result = R_avg[:, :, 0].T  # (N, P) - transpose for output format
    # Set infinite persistence length for singular systems
    result = result.copy()
    result[np.abs(dets.T) < 1e-10] = np.inf
    if plot:
        if P == 1 and N == 1:
            # report
            print("-------------- Calculation Report -------------")
            print(f"Temperature: {Ts[0]:.2f} K")
            print(f"Correlation Length: {result[0, 0]:.6f}")
            print("-----------------------------------------------")
        elif P == 1:
            # 1D plot: persistence vs temperature (single composition)
            lp_1d = result[:, 0]
            ...
            if not np.all(finite_mask):
                pass
            format_subplot("Temperature (K)", "Persistence Length (Å)",
                           "Persistence Length vs Temperature")
            plt.show()
        elif N == 1:
            # Fixed T, vary composition → 1D curve: lp vs composition
            lp_1d = result[0, :]  # shape (P,)
            ...
            plt.plot(x[finite], lp_1d[finite], 'o-')
            format_subplot(
                "Probability of Repeat Unit 1", "Persistence Length (Å)",
                f"Persistence Length vs Composition (T = {Ts[0]:.2f} K)")
            plt.show()
        else:
            lp_plot = result.copy()
            lp_plot[np.isinf(lp_plot)] = np.nan  # Mask inf for display
            ...
            cbar = plt.colorbar(im)
            cbar.set_label("Persistence length (Å)",
                           fontsize=14,
                           fontfamily="Helvetica")
            cbar.ax.tick_params(labelsize=14)
            plt.setp(cbar.ax.get_yticklabels(), fontfamily="Helvetica")
